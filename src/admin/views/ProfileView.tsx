...
import { Mail, Phone, MapPin, Building2, Calendar, Edit, Save, Camera, X } from "lucide-react";
        ...
        <div className="flex flex-col gap-4 px-6 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
            <div className="relative group">
              <div className="-mt-10 grid size-24 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-3xl font-bold text-white ring-4 ring-card shadow-elevated transition-transform">
                {profile.initials}
              </div>
              {isEditing && (
                <button 
            ...
            <div className="mb-1">
              {isEditing ? (
                <div className="flex items-center gap-3">
                  ...
                  <Input 
                    ...
                    onChange={(e) => setTempProfile({...tempProfile, initials: e.target.value.substring(0,2).toUpperCase()})}
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <Badge variant="outline" className="rounded-full border-blue-500/30 bg-blue-500/10 text-[11px] font-bold tracking-wider uppercase text-blue-600 dark:text-blue-400">System Admin</Badge>
                <Badge variant="outline" className="rounded-full text-[11px] font-bold tracking-wider uppercase">Full Access</Badge>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="rounded-2xl border bg-card p-6 shadow-soft">
              ...
              <div className="space-y-4 text-sm">
                <div className="flex justify-between items-center border-b pb-2"><span className="text-muted-foreground font-medium">Account ID</span><span className="font-bold">ADM-001</span></div>
                <div className="flex justify-between items-center border-b pb-2"><span className="text-muted-foreground font-medium">Role Level</span><span className="font-bold text-blue-600">Super Admin</span></div>
                <div className="flex justify-between items-center border-b pb-2"><span className="text-muted-foreground font-medium">Joined Date</span><span className="font-bold">{profile.joined}</span></div>
                <div className="flex justify-between items-center"><span className="text-muted-foreground font-medium">Last Login</span><span className="font-bold text-emerald-600">Today, 08:30 AM</span></div>
              </div>
            </div>
          </div>
        </TabsContent>
